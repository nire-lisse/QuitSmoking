...
local function checkQuitSmoking()
    ...
    if player:hasModData() and player:getModData().chancetoquit ~= nil or player:getModData().incremental ~= nil then
        return
    else
        player:getModData().chancetoquit = 0
        player:getModData().incremental = 1.0
...
local function quitChanceUpdate(_player, _playerdata)
    ...
    local newchance = 0
    if player:HasTrait("Smoker") and playerdata.chancetoquit <= 1 then
        ...
        elseif player:HasTrait("Unlucky") then
            ...
            newchance = ZombRand(1, 100) / 67200
    ...
    if player:getTimeSinceLastSmoke() <= 1 then
        playerdata.chancetoquit = 0
...
local function smokerUpdate()
    ...
    if player:HasTrait("Smoker") and magicnumber == 0 then
    ...
    if playerdata.formersmoker == true and player:getTimeSinceLastSmoke() <= 9 then
        playerdata.chancetoquit = 0
